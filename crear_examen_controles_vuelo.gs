...
function crearEvaluacionCompleta() {
  ...
  form.setDescription(
    ...
    'SECCIÓN 2: Opción múltiple A320 — 20 preguntas (5 pts c/u = 100 pts)\n' +
    'SECCIÓN 3: Verdadero/Falso B737 — 20 preguntas (2 pts c/u = 40 pts)\n' +
    'SECCIÓN 4: Caso de estudio — 20 preguntas (3 pts c/u = 60 pts)\n' +
    'PUNTAJE TOTAL: 200 puntos\n\n' +
    'Calificación automática.'
  );
  
  form.setIsQuiz(true);
  form.setCollectEmail(false); // No necesitamos email ya que tenemos matrícula
  form.setProgressBar(true);
  form.setLimitOneResponsePerUser(true);
  
  // ================================================================
  // SECCIÓN 0: DATOS DEL ALUMNO
  // ================================================================
  
  form.addSectionHeaderItem().setTitle('DATOS DEL ALUMNO');
  
  var nombreItem = form.addTextItem();
  nombreItem.setTitle('Nombre completo del alumno:');
  nombreItem.setRequired(true);
  
  var grupoItem = form.addTextItem();
  grupoItem.setTitle('Grupo:');
  grupoItem.setRequired(true);
  
  var matriculaItem = form.addTextItem();
  matriculaItem.setTitle('Matrícula:');
  matriculaItem.setRequired(true);
