...
\begin{itemize}
  \item \textbf{Search coverage:} The review is limited to English-language publications in three databases (IEEE Xplore, ScienceDirect, SpringerLink). Relevant work published in Chinese---where agricultural UAV research is particularly active---Spanish, or Portuguese, or indexed in AGRIS or CAB Abstracts, is not captured. Future iterations should include at minimum Scopus and Web of Science for broader coverage. Notably, MDPI journals (\textit{Drones}, \textit{Remote Sensing}), which are among the highest-volume publication venues for agricultural UAV research post-2020, are not indexed in the three databases used and are therefore absent from this corpus; this constitutes a structural source-level bias that goes beyond the generic Scopus/WoS limitation.
