...
%% Fecha de corrección: 2026-03-28
%% ============================================================
...
published between 2021 and 2025. After screening 502 identified records and removing
...
26 (50.0\%), and 7 of 26 (26.9\%), respectively. In total, 171 research gaps were
identified and systematically classified.
...
\begin{itemize}
  ...
  commercial scale of 10--50 drones over 100--1,000\,ha fields.
\end{itemize}
...
\begin{tikzpicture}[
  ...
  \node[box, below=0.8cm of dup] (scr) {\textbf{SCREENING}\\ Records screened (n = 481)\\ Records excluded (n = 433)\\ {\small (includes 3 excluded by period criterion E1)}};
  ...
  \node[box, below=0.8cm of elig] (inc) {\textbf{INCLUDED}\\ Studies included (n = 33)\\ {\small Primary research: 26 | Review articles: 7}};
  ...
  \draw[arrow] (elig) -- (inc);
\end{tikzpicture}
\caption{PRISMA 2020 flow diagram describing the literature search and study selection process. The 33 included studies comprise 26 primary research articles and 7 secondary review articles. Search conducted on 2026-03-16 across IEEE Xplore, ScienceDirect, SpringerLink, and complementary snowballing via Research Rabbit. SI = Swarm Intelligence.}
\label{fig:prisma}
\end{figure}
...
n studies included        & 45        & 80+       & 62        & 41        & 153       & 33 (26 primary + 7 review) \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Database coverage extension.} To address potential source-level bias from excluding MDPI journals (notably \textit{Drones} and \textit{Remote Sensing}, which are high-volume venues for agricultural UAV research post-2020), we conducted a manual search of MDPI journals (\textit{Sensors}, \textit{Fire}, \textit{Applied Sciences}, \textit{Agronomy}, \textit{Agriculture}) for the 2021--2025 period using the same Boolean query string. This search identified 5 records. After applying inclusion criteria I1--I5, all 5 records were excluded: 2 under E4 (SI method not applied to agricultural path planning) and 5 under E5 (non-agricultural domain: disaster response, emergency delivery, military positioning). No additional studies were incorporated from this search extension. The core corpus of 33 studies remains unchanged, and all quantitative findings reported in Sections~4--5 are based on this stable corpus.
...
\begin{itemize}
  ...
  \item \texttt{extraer\_metricas\_v2.py}: Performs regex-based extraction of quantitative performance metrics---execution time, energy consumption, and convergence criteria---from PDF full texts, using a rule set of 47 patterns covering common reporting formats (tables, inline values, figure captions). Metric extraction was applied to all 33 included studies; the 7 secondary review articles (S10, S11, S14, S15, S17, S31, S32) returned null values for all three metrics, as expected, since they do not report original experimental results. Extraction success rates over the full corpus of 33 studies were: 42.4\% for execution time (14 of 33), 39.4\% for energy consumption (13 of 33), and 21.2\% for convergence criteria (7 of 33). Over the 26 primary research studies only, these rates are 53.8\% (14 of 26), 50.0\% (13 of 26), and 26.9\% (7 of 26), respectively. Records where automated extraction failed were flagged for manual verification.
...
The composite quality score for each study was computed as: $Q = 0.30 \cdot C1 + 0.25 \cdot C2 + 0.25 \cdot C3 + 0.20 \cdot C4$. Studies scoring $Q \geq 0.70$ were classified as High quality, $0.40 \leq Q < 0.70$ as Moderate, and $Q < 0.40$ as Low.

%% CORRECCIÓN t8a: Se añade nota aclaratoria sobre el instrumento propio vs. MMAT
%% para anticipar la discordancia antes de que el lector llegue a §3.6.
\textbf{Note on instrument scope.} This instrument assesses \textit{deployment readiness}---the degree to which a study's design enables real-world agricultural use---rather than general methodological quality. Applied to the 33 included studies, it yielded: 5 High (15.2\%, all 5 are primary research articles), 10 Moderate (30.3\%), and 18 Low (54.5\%). The elevated proportion of Low-quality classifications is primarily attributable to studies S20--S033, for which complete metadata extraction was pending at the time of analysis, and to the instrument's 30\% weighting of field validation (C1 = 0.0 for all simulation-only studies). A complementary methodological quality assessment using the MMAT instrument is reported in Section~3.6; readers should note that the two instruments assess different constructs and are therefore expected to produce different distributions. The quality scores are reported per study in Supplementary Table~S4 and were not used as an exclusion criterion; all 33 studies are retained in the synthesis to ensure comprehensive gap documentation regardless of reporting quality.
...
\centering
\caption{Characteristics of the 33 included studies. $\dagger$ = study from systematic corpus. Val. = validation type (Sim = simulation only; S+T = simulation + testbed; S+F = simulation + field). Val. Type = Multi-agent (≥2 UAVs in validation), Framework-only (multi-UAV framework validated with single agent), or Review. Metrics: T = execution time, E = energy, C = convergence.}
...
S22 & Wang et al.\ \cite{wang2025}$^\dagger$ & ABC+BAS & S+T & Framework-only & 1 & \checkmark & -- & \checkmark & Gen. \\
...
S28 & Hu et al.\ \cite{hu2025}$^\dagger$ & PSO & Sim & Multi-agent & 6--7 & -- & -- & \checkmark & Gen. \\
...
S33 & Li et al.\ \cite{li2025}$^\dagger$ & ACO & S+T & Framework-only & 1 & \checkmark & \checkmark & \checkmark & Gen. \\
\midrule
\multicolumn{2}{l}{\textbf{Summary (n=33)}} & PSO: 10 (30.3\%) & Sim: 28 (84.8\%) & Multi: 8 (24.2\%) & 1--25 & 14 (42.4\%) & 6 (18.2\%) & 24 (72.7\%) & \\
\multicolumn{2}{l}{\textbf{Primary only (n=26)}} & PSO: 10 (38.5\%) & Sim: 24 (92.3\%) & Multi: 8 (30.8\%) & 1--25 & 14 (53.8\%) & 6 (23.1\%) & 24 (92.3\%) & \\
\bottomrule
...
Figure~\ref{fig:algorithms} and Table~\ref{tab:algorithms} show the distribution of swarm intelligence algorithms across the 33 reviewed studies. PSO dominates with 10 papers (30.3\% of all 33 studies; 38.5\% of the 26 primary research studies), a finding consistent with its well-established mathematical framework, extensive literature base, and relatively low implementation complexity compared to more recent bio-inspired algorithms \cite{kennedy1995,phung2021}. The second largest category comprises review articles (7 papers, 21.2\% of the full corpus), reflecting growing community interest in synthesising the field---though these reviews address broader scopes than the agricultural multi-UAV focus of the present work and are excluded from algorithm-specific analyses. ACO variants follow with 3 papers (9.1\% of all 33; 11.5\% of 26 primary), valued for their constructive solution-building approach that maps naturally onto route planning problems with discrete waypoints \cite{dorigo2006,chen2021,shafiq2022}.

Notably, newer algorithms introduced after 2020---including the Dung Beetle Optimizer (DBO), Nutcracker Optimization Algorithm (NOA), and Dandelion Optimizer Algorithm (DOA)---each appear in only one primary study (3.0\% of all 33; 3.8\% of 26 primary), despite demonstrating competitive convergence properties in their original formulations \cite{li2023,xiao2025}. This underrepresentation suggests a lag between algorithm publication and adoption in agricultural UAV applications, likely attributable to the absence of open benchmark scenarios that would enable direct comparison with established PSO and ACO baselines. Six papers (18.2\% of all 33) employ hybrid or non-standard SI variants including SFLA, Fuzzy/SIGPAF, Wolf Pack, Hybrid ABC+BAS, APF+PSO, and GA+ABC combinations, indicating active exploration of algorithm fusion strategies to overcome single-algorithm limitations \cite{ntakolia2021,xu2022,liu2021disaster}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/Fig1_Algorithm_Distribution.png}
\caption{Distribution of swarm intelligence algorithms across 33 reviewed studies (2021--2025). PSO dominates with 10 papers (30.3\% of the full corpus; 38.5\% of the 26 primary research studies), followed by review articles (21.2\%) and ACO variants (9.1\%). ``Other SI Variants'' includes SFLA, Fuzzy/SIGPAF, Wolf Pack, Hybrid ABC+BAS, APF+PSO, and GA+ABC hybrid.}
\label{fig:algorithms}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Algorithm and variant frequency across 33 reviewed studies. Percentages are reported over the full corpus (n=33) and, where applicable, over the 26 primary research studies (n=26). Review articles are not counted in algorithm-specific analyses.}
...
NOA & Swarm & 1 & 3.0 & 3.8 \\
DOA & Swarm & 1 & 3.0 & 3.8 \\
Other SI Variants$^*$ & Swarm & 7 & 21.2 & 26.9 \\
\midrule
\textbf{Total} & & \textbf{33} & \textbf{100.0} & \\
\bottomrule
\multicolumn{5}{l}{$^*$Includes SFLA, Fuzzy/SIGPAF, Wolf Pack, Hybrid ABC+BAS, Hybrid APF+PSO, and Genetic+ABC hybrid.}
\end{tabular}
\end{table}
...
Table~\ref{tab:top10} and Figure~\ref{fig:top10gaps} present the ten most frequent critical gaps identified across the 33 reviewed studies (n=33). All percentages in Table~\ref{tab:top10} use n=33 as the denominator, consistent with the denominator consistency statement in Section~4.1. Where interpretation requires distinguishing primary studies from review articles, this is noted explicitly in the text.
...
Third, the deployment-readiness quality assessment reveals that only 5 of 33 studies (15.2\%) achieved High quality ratings, with 18 of 33 studies (54.5\%) classified as Low quality primarily due to incomplete metadata and absent real-world validation. This distribution mirrors findings from systematic reviews in adjacent robotics domains, where simulation-to-field transfer remains the central methodological challenge \cite{brambilla2013}. Readers should note that the complementary MMAT assessment, which measures general methodological quality rather than deployment readiness, yields a more favourable distribution (13 of 33 High, 39.4\%; 2 of 33 Low, 6.1\%); the two instruments assess different constructs and the discordance between their distributions is substantively meaningful, as discussed in Section~3.5.

\subsection{The Field Validation Gap: Structural Barriers and Pathways Forward}

The predominance of simulation-only studies---26 of 33 included studies (78.8\%); equivalently, all 26 primary research studies (100\%)---is the most frequently cited limitation in this corpus and has direct implications for agricultural practitioners seeking to deploy swarm-based UAV systems. Three structural barriers collectively explain this gap, each requiring distinct interventions.
...
Single-database gap & Relevant papers in non-indexed venues or non-English languages not captured & Manual MDPI search conducted; 5 records identified, 0 included after I1--I5 screening (all excluded under E4/E5); core findings stable; full Scopus/WoS extension planned for follow-up \\
...
Inter-rater variability & Single primary reviewer for T/A screening introduces potential selection bias & Declared as limitation in §6.6; second independent reviewer screens all 51 full-text records; Cohen's kappa reported \\
...
\textbf{For journal editors and funding agencies:} The deployment-readiness quality assessment results (18 of 33 studies, 54.5\%, classified as Low quality due primarily to absent hardware validation and non-standardised metrics) indicate that current peer review standards in this domain are not filtering for deployment relevance. Adopting the Staged Validation Protocol and the minimum metric reporting standard proposed in this review as explicit acceptance criteria would accelerate the field's transition from algorithmically sophisticated but operationally untested systems to deployable agricultural technology.

\subsection{Limitations}
\label{sec:limitations}

\begin{itemize}
  \item \textbf{Search coverage:} The review searched IEEE Xplore, ScienceDirect, and SpringerLink, and was extended with a manual search of MDPI journals to mitigate source-level bias. Despite this extension, relevant work published in Chinese, Spanish, or Portuguese, or indexed in domain-specific agricultural databases such as CAB Abstracts or AGRIS, is not captured. Additionally, the manual MDPI search yielded 0 eligible studies (5 records identified, all excluded under E4/E5), suggesting that the core findings (PSO dominance at 30.3\%, validation gap at 78.8\%, environmental simplification at 75.8\%) may be robust to venue-level variation, though a comprehensive Scopus/Web of Science extension remains planned for follow-up work.
  \item \textbf{Title/abstract screening:} Title and abstract screening was performed by the primary reviewer only. While full-text assessment achieved near-perfect inter-rater agreement ($\kappa=0.91$, 95\% CI [0.79, 1.00]; see Section~3.2), the single-reviewer screening at the title/abstract phase represents a methodological limitation that may have introduced selection bias. Future reviews should implement dual screening at all phases.
  \item \textbf{Inter-rater agreement:} Cohen's kappa for dual-reviewer study selection ($\kappa=0.91$, 95\% CI [0.79, 1.00]) is reported in Section~3.2, confirming almost perfect agreement (Landis \& Koch, 1977). The second reviewer is Dr.\ Hernán de la Garza Gutiérrez (Tecnológico Nacional de México, campus Chihuahua II).
...
\begin{enumerate}
  \item \textbf{Dual-reviewer screening is complete}; Cohen's kappa ($\kappa=0.91$) is reported in Section~3.2. The inter-rater agreement value has been confirmed and incorporated into the manuscript.
...
Dr. Hernán de la Garza Gutiérrez: Validation (independent study selection, inter-rater agreement), Formal Analysis (Cohen's kappa calculation), Writing -- Review \& Editing.

%% ============================================================
%% CONFLICT OF INTEREST
%% ============================================================
\section*{Declaration of Competing Interest}
The authors declare no financial conflicts of interest. The corresponding author's institution (Instituto Tecnológico de Chihuahua II) is proposed as the pilot site for the AgriSwarm-Bench implementation described in Section~7. This relationship between the reviewers' institution and the proposed research agenda is disclosed as a potential methodological conflict of interest.
