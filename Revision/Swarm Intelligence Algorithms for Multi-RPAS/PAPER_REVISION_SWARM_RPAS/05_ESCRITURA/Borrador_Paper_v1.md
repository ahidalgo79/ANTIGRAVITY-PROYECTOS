...
- First systematic review quantifying the simulation-to-reality 
  gap (78.8%) in swarm intelligence-based agricultural UAV research
- 171 research gaps documented across four dimensions: 
  Technological, Practical, Methodological, and Theoretical
- Staged Validation Protocol proposed to bridge simulation 
  and real-field deployment
- AgriSwarm-Bench framework proposed as open community 
  benchmark for SI-UAV agricultural systems
- PRISMA-compliant synthesis of 33 studies from 502 identified 
  records (2021–2025)
...
**Methods:** Following PRISMA guidelines, a systematic literature search was conducted across IEEE Xplore, ScienceDirect, SpringerLink for studies published between 2021 and 2025. After screening 502 identified records and removing 21 duplicates, 33 relevant studies were included. Data were extracted regarding algorithm type, evaluation metrics, environmental modeling, and experimental validation. Identified limitations were categorized into four dimensions: technological, practical, methodological, and theoretical.
...
- **Validation gap:** 78.8% lack hardware validation in real field conditions
...
**Figure 1.** PRISMA flow diagram describing the literature search and study selection process for this systematic review.
    ...
    • ScienceDirect: n = 196
    • SpringerLink: n = 254
    • Snowballing via Research Rabbit (citation tracking from seed papers): n = 20
                    |
                    v
                    SCREENING
...
Additional records were identified through citation snowballing 
using Research Rabbit, starting from five seed papers identified 
in the initial database search. This complementary strategy 
yielded 20 additional records not captured by the primary 
database queries. The screening process followed the PRISMA 
2020 guidelines and is summarized in Figure 1. After initial identification of 502 records, duplicate removal yielded 481 unique studies for title/abstract screening. Following exclusion of 430 non-relevant records, 51 full-text articles were assessed for eligibility, resulting in 33 studies included in the final synthesis.
...
|-----------|--------|---------|
| C1: Experimental validation | 30% | Real field=1.0, Sim+Exp=0.5, Sim only=0.0 |
| C2: Quantitative metrics reported | 25% | All 3 metrics=1.0, 2=0.67, 1=0.33, None=0.0 |
| C3: Comparison with baseline algorithm | 25% | Yes=1.0, No=0.0 |
| C4: Agricultural context described | 20% | Yes=1.0, No=0.0 |

Studies scoring ≥0.70 were classified as High quality, 
0.40–0.69 as Moderate, and <0.40 as Low. 
Since this review was conducted by a single author, 
quality assessment was performed independently and verified 
against paper abstracts to mitigate subjectivity bias.

Results of the quality assessment are summarized below:

| Quality Level | Studies | Percentage |
|---------------|---------|------------|
| High (≥0.70) | 5 | 15.2% |
| Moderate (0.40–0.69) | 10 | 30.3% |
| Low (<0.40) | 18 | 54.5% |

The high proportion of Low quality studies (54.5%) is 
largely attributable to papers 020–034, which were 
identified through snowballing and for which complete 
metadata extraction was pending at the time of analysis. 
This represents a limitation of the current review and 
is addressed in Section 8.2. Among fully analyzed studies 
(papers 001–019), quality distribution was more balanced: 
5 High (26.3%), 10 Moderate (52.6%), and 4 Low (21.1%).
---
...
Table 1: Algorithm and Variant Frequency Across 33 Reviewed Studies
...
| NOA | 1 | 3.0% | Swarm |
| DOA | 1 | 3.0% | Swarm |
| Other SI Variants* | 6 | 18.2% | Swarm |
| **TOTAL** | **33** | **100%** | |

*Includes: SFLA (PAPER_008), Fuzzy Logic/SIGPAF (PAPER_007), 
Wolf Pack Algorithm (PAPER_022), Hybrid ABC+BAS (PAPER_023), 
Hybrid APF+PSO (PAPER_030), and Genetic+ABC hybrid (PAPER_004).
...
Gap prioritization was performed using a binary impact matrix. 
A gap was classified as Critical if it simultaneously met 
two conditions: (1) it was present in more than 30% of 
analyzed studies (>10 papers), AND (2) its absence directly 
prevents real-world field deployment (e.g., hardware 
validation, wind modeling, real-time replanning). Gaps 
present in fewer than 30% of studies but with significant 
theoretical impact were classified as Important, while those 
with low frequency and limited deployment impact were 
classified as Minor.

Figure 4 shows the prioritization of gaps by criticality 
level. Critical gaps represent 44.4% (76 gaps) of the total, 
indicating substantial barriers to real-world deployment of 
swarm-based agricultural UAV systems.
...
**Field Validation Gap (78.8%):** The predominance of 
simulation-only studies is not merely a methodological 
preference — it reflects real barriers including the high 
cost of agricultural UAV platforms (typically USD 2,000–
15,000 per unit for research-grade systems), regulatory 
restrictions on beyond-visual-line-of-sight (BVLOS) 
operations in most jurisdictions, and the logistical 
complexity of coordinating multi-UAV field experiments 
in active agricultural environments. Until these barriers 
are systematically addressed through open testbeds and 
regulatory sandboxes, simulation results will continue to 
dominate the literature with limited transferability to 
real deployments [28].

To bridge this gap, we propose that future studies adopt 
a Staged Validation Protocol before claiming field 
applicability:

**Stage 1 — Simulation:** Algorithm validation in 
controlled virtual environments with realistic agricultural 
parameters (field geometry, wind profiles, crop row 
constraints).

**Stage 2 — Testbed:** Hardware-in-the-loop validation 
using physical UAV platforms in controlled indoor or 
outdoor spaces without crop interference.

**Stage 3 — Controlled Field:** Full deployment in a 
real agricultural field under supervised conditions, 
reporting standardized metrics (coverage rate, energy 
consumption per hectare, mission completion time).

This protocol mirrors validated approaches in adjacent 
domains such as search and rescue robotics [32] and 
provides a reproducible pathway from algorithmic 
contribution to practical deployment.

**Environmental Modeling Gap (75.8%):** Wind significantly 
affects UAV energy consumption, trajectory accuracy, and 
inter-agent communication in open-field operations. The 
absence of wind and weather modeling in 75.8% of studies 
means that reported performance metrics are likely 
optimistic relative to real agricultural conditions. This 
gap is particularly critical for spraying applications, 
where wind drift directly affects pesticide distribution 
accuracy and regulatory compliance [29]. Furthermore, 
LiPo battery performance degrades significantly in high 
ambient temperatures common in agricultural regions 
(>35°C), a factor entirely absent from current models.

**Metric Inconsistency:** The lack of standardized reporting 
(only 42.4% report execution time, 39.4% energy consumption) 
prevents meaningful cross-study comparison and makes it 
impossible for agricultural practitioners to objectively 
select algorithms for specific deployment scenarios. This 
mirrors a broader challenge in the swarm robotics community 
identified by prior reviews [26,27], but is particularly 
acute in agriculture where operational constraints 
(battery life, coverage area per charge, spraying 
uniformity index) require domain-specific benchmarks 
that currently do not exist.

### 6.2 Comparison with Other Domains

Only 21.2% of reviewed studies include any form of hardware 
or experimental validation, while 78.8% rely exclusively on 
simulation, theoretical analysis, or do not specify their 
validation method. This suggests that agricultural UAV research 
lags behind other application domains in experimental 
validation. This gap likely reflects domain-specific barriers 
including regulatory restrictions on BVLOS flights, cost 
sensitivity of agricultural operations, and challenging 
environmental conditions such as dust, wind, and GPS 
interference that complicate real-field experiments.

Future comparative studies should systematically quantify 
validation rates across domains (military, search and rescue, 
infrastructure inspection) to establish empirical benchmarks 
for the agricultural robotics community.
...
### 7.1 Priority Directions for the Research Community
...
establishment of AgriSwarm-Bench: an open, standardized 
benchmarking repository for swarm intelligence algorithms 
applied to agricultural UAV path planning. AgriSwarm-Bench 
would provide:

- Standardized agricultural field scenarios (crop rows, 
  irregular boundaries, wind profiles) for simulation
- Unified evaluation metrics (coverage rate, energy per 
  hectare, mission completion time, scalability index)
- Open datasets from real field experiments to enable 
  reproducible cross-algorithm comparison
- A community leaderboard tracking algorithm performance 
  across scenario types

This initiative mirrors successful benchmarking efforts 
in adjacent robotics domains and addresses the metric 
inconsistency gap identified in 54.5% of reviewed studies. 
The framework proposed here is currently being implemented 
as part of ongoing doctoral research at Instituto 
Tecnológico de Chihuahua II, which will serve as the 
first pilot validation case for the AgriSwarm-Bench 
protocol using a heterogeneous multi-UAV platform.

### 7.2 Proposed Community Roadmap (Short-to-Mid Term)

| Phase | Timeframe | Activity | Deliverable |
|-------|-----------|----------|-------------|
| 1 | Short-term (0–12 months) | Implement benchmark with 3–4 representative SI algorithms | Open benchmark framework |
| 2 | Short-term (0–12 months) | Configure experimental platform with 3–5 UAVs | Replicable test environment |
| 3 | Mid-term (1–2 years) | Integrate real-time replanning under dynamic conditions | Enhanced algorithm + dataset |
| 4 | Mid-term (1–2 years) | Execute comparative experiments across domains | Peer-reviewed results |
| 5 | Mid-term (2–3 years) | Dissemination and benchmark standardization | Community guidelines + public dataset |

While this roadmap aligns with ongoing doctoral research 
efforts at Instituto Tecnológico de Chihuahua II, it is 
proposed as a baseline for the broader agricultural robotics 
community to establish common benchmarks and accelerate 
real-world deployment of swarm-based UAV systems.
---
...
2. **78.8% lack hardware validation**
...
[26] Puente-Castro A, Rivero D, Pazos A, Fernandez-Blanco E. 
A review of artificial intelligence applied to path planning 
in UAV swarms. Neural Computing and Applications 
2022;34:153-170. doi:10.1007/s00521-021-06569-4

[27] Sharma A, Shoval S, Sharma A, Pandey JK. Path planning 
for multiple targets interception by the swarm of UAVs based 
on swarm intelligence algorithms: A review. IETE Technical 
Review 2022;39:524-538. doi:10.1080/02564602.2021.1894250

[28] Phung MD, Ha QP. Safety-enhanced UAV path planning with 
spherical vector-based particle swarm optimization. Applied 
Soft Computing 2021;107:107376. 
doi:10.1016/j.asoc.2021.107376

[29] Chen J, Ling F, Zhang Y, You T, Liu Y, Du X. Coverage 
path planning of heterogeneous unmanned aerial vehicles based 
on ant colony system. Swarm and Evolutionary Computation 
2021;66:101005. doi:10.1016/j.swevo.2021.101005

[30] Liu Q, Zhang Y, Li M, Zhang Z, Cao N, Shang J. Multi-UAV 
path planning based on fusion of sparrow search algorithm and 
improved bioinspired neural network. IEEE Access 
2021;9:120946-120960. doi:10.1109/ACCESS.2021.3109879

[31] Pan N, Zhang M, Sun Y, Chen S, Liu H, Guo X. Study on 
border patrol task planning of heterogeneous UAVs group based 
on swarm intelligence. Science Progress 2022;105. 
doi:10.1177/00368504221094722

[32] Shafiq M, Ali ZA, Israr A, Alkhammash EH, Hadjouni M, 
Jussila JJ. Convergence analysis of path planning of 
multi-UAVs using max-min ant colony optimization approach. 
Sensors 2022;22:5395. doi:10.3390/s22145395

[33] Ait Saadi A, Soukane A, Meraihi Y, Gabis AB, Mirjalili S, 
Ramdane-Cherif A. UAV path planning using optimization 
approaches: A survey. Archives of Computational Methods in 
Engineering 2022;29:4233-4284. 
...
**Figures:** 5 (separate PNG files at 300 DPI)  
